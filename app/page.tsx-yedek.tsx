...
<div className="relative">
  ...
  setGateFocus(false)
  setTimeout(() => {
    document.getElementById("kadim-taslar")?.scrollIntoView({
      behavior: "smooth",
      block: "start",
    })
  }, 0)
...
<section id="featured-artifact" className="relative z-10 min-h-[55vh] flex items-center px-10 md:px-24 py-10">
        ...
        <div className="overflow-hidden rounded-2xl transition-all duration-500 group-hover:shadow-[0_0_60px_rgba(198,161,91,0.30)]">
          ...
          <img
            ...
            onClick={() => {
              ...
              setSelectedImage(item.image)
              setTimeout(() => {
                document.getElementById("featured-artifact")?.scrollIntoView({
                  behavior: "smooth",
                  block: "center",
                })
              }, 0)
...
<div className={`${fullscreen ? "hidden" : "max-w-xl w-full min-w-0"}`}>
  ...
  {selectedSection === "kadimtaslar" ? "Taş Kaydı" : "Dokuma Kaydı"}
</div>
...
>
  ...
  stones[selectedStone as keyof typeof stones].map((item) => (

  <div
    key={item.image}
    className="flex-shrink-0 w-[160px] flex flex-col items-center gap-1"
  >
